...
public interface AsteroidRepository extends JpaRepository<Asteroid, Long> {
    ...
    @Query("SELECT a FROM Asteroid a WHERE a.band.id = :bandId ORDER BY a.mass DESC")
    List<Asteroid> findByBandIdOrderByMassDesc(@Param("bandId") Long bandId);

    @Query("SELECT a FROM Asteroid a WHERE a.band.id = :bandId AND a.isNotable = true ORDER BY a.mass DESC")
